...
class CRM_Kavaconvertmemberships_Membership {
  ...
  public static function convertMeewerkendLid($contactId = NULL) {
    ...
    foreach ($memberships as $membership) {
      ...
      // reset is override
      CRM_Core_DAO::executeQuery("update civicrm_membership set is_override = 0 where id = " . $m[0]['id']);
    }
  }

  public static function convertCorresponderendLid($contactId = NULL) {
    $currentYear = (int)date('Y');
    $nextYear = $currentYear + 1;
    ...
    $memberships = \Civi\Api4\Membership::get(FALSE)
      ->addSelect('*', 'custom.*')
      ->addWhere('membership_type_id', '=', CRM_Kavaconvertmemberships_MembershipType::CORRESPONDERENDE_LEDEN)
      ->addWhere('end_date', '>', "$currentYear-12-31");
